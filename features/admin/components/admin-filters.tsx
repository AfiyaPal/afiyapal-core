...
export const reusableAdminFilters = {
  ...
  status: { key: "status", label: "Status", type: "select", options: [{ value: "", label: "All statuses" }, { value: "active", label: "Active" }, { value: "pending", label: "Pending" }, { value: "in-review", label: "In review" }, { value: "resolved", label: "Resolved" }, { value: "suspended", label: "Suspended" }] },
...
export function AdminFilters({ filters }: { filters: readonly AdminFilterConfig[] }) {
  return (
    <form className="grid gap-3 rounded-3xl border border-emerald-100 bg-white p-4 shadow-sm md:grid-cols-2 xl:grid-cols-4">
      {filters.map((filter) => filter.type === "select" ? (
        <label key={filter.key} className="space-y-2 text-sm font-bold text-slate-700"><span>{filter.label}</span><select name={filter.key} className="w-full rounded-2xl border border-emerald-100 bg-white px-4 py-3 text-sm font-medium text-slate-700 outline-none transition focus:border-brand-600 focus:ring-4 focus:ring-emerald-100">{filter.options?.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
      ) : (
        <label key={filter.key} className="space-y-2 text-sm font-bold text-slate-700"><span>{filter.label}</span><input name={filter.key} type={filter.type === "date" ? "date" : "search"} placeholder={filter.placeholder} className="w-full rounded-2xl border border-emerald-100 bg-white px-4 py-3 text-sm font-medium text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-brand-600 focus:ring-4 focus:ring-emerald-100" /></label>
      ))}
    </form>
  );
}
